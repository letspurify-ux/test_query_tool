...
/
BEGIN EXECUTE IMMEDIATE 'DROP TYPE oqt_row'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE oqt_tmp_result PURGE'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE oqt_run_log PURGE'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE oqt_emp'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
CREATE TABLE oqt_emp (
  emp_id   NUMBER       CONSTRAINT pk_oqt_emp22 PRIMARY KEY,
  emp_name VARCHAR2(50) NOT NULL,
  sal      NUMBER       NOT NULL
);

INSERT INTO oqt_emp(emp_id, emp_name, sal) VALUES (100, 'ALICE',  5000);
INSERT INTO oqt_emp(emp_id, emp_name, sal) VALUES (101, 'BOB',    7000);
INSERT INTO oqt_emp(emp_id, emp_name, sal) VALUES (102, 'CAROL',  9000);
INSERT INTO oqt_emp(emp_id, emp_name, sal) VALUES (103, 'DAVE',  12000);
COMMIT;
...
CREATE OR REPLACE TYPE oqt_row AS OBJECT (
  emp_id   NUMBER,
...
CREATE OR REPLACE TYPE oqt_row_tab AS TABLE OF oqt_row;
/
...
CREATE OR REPLACE PACKAGE BODY oqt_demo_pkg AS
  ...
  BEGIN
    INSERT INTO oqt_run_log(run_id, msg) VALUES (p_run_id, p_msg);
  END;
  ...
  BEGIN
    FOR r IN (SELECT emp_id, emp_name, sal
               ...
               ORDER BY sal)
    LOOP
      PIPE ROW(oqt_row(r.emp_id, r.emp_name, r.sal));
    END LOOP;
    RETURN;
  END;
  ...
  BEGIN
    ...
    LOOP
      v_row_no := v_row_no + 1;
      INSERT INTO oqt_tmp_result(run_id, row_no, payload)
...
END;
/
SELECT * FROM oqt_run_log ORDER BY run_ts DESC;
...
END;
/
SELECT * FROM oqt_emp ORDER BY emp_id;
...
BEGIN
  ...
  LOOP
    FETCH rc INTO v_emp_id, v_emp_name, v_sal;
...
SELECT oqt_demo_pkg.func_get_sal(100) AS sal_100 FROM dual;

SELECT emp_id, emp_name, oqt_demo_pkg.func_get_sal(emp_id) AS sal_by_func
  FROM oqt_emp
 ORDER BY emp_id;
...
DECLARE
  v_run_id NUMBER := 20260127; -- 원하는 run_id로 바꿔도 됨
BEGIN
...
SELECT * FROM oqt_tmp_result WHERE run_id = 20260127 ORDER BY row_no;

SELECT * FROM oqt_run_log WHERE run_id = 20260127 ORDER BY run_ts DESC;
